  ...
  const app = {
    ...
    generateHTMLForConcert(concert) {
      ...
      <div class="concert" data-id="${concert.id}">
        ...
        <div class="concert__meta">
          <span class="date__from">${new Date(
            concert.from
          ).toDateString()}</span>
        </div>
      </div>
    ...
    generateHTMLForConcertDetails(concert) {
      ...
      <div class="concert-details__right">
        <div class="concert-details__meta">${new Date(
          concert.from
        ).toDateString()}</div>
    ...
    generateHTMLForSocials(socials) {
      ...
      for (let prop in socials) {
        tmpStr += `<li class="socials__list-item"><a href="${
          socials[prop]
        }" target="__blank" rel="noreferrer nofollow">${this.generateHTMLForSocialMediaIcon(
          prop
        )}</i><span>${prop}</span></a></li>`;
      }
      return tmpStr;
    },
    generateHTMLForSocialMediaIcon(socialMedia) {
      switch (socialMedia) {
        case "website":
        default:
          return '<i class="bi bi-globe">';
        case "youtube":
          return '<i class="bi bi-youtube">';
        case "spotify":
          return '<i class="bi bi-spotify">';
        case "facebook":
          return '<i class="bi bi-facebook">';
        case "instagram":
          return '<i class="bi bi-instagram">';
        case "twitter":
          return '<i class="bi bi-twitter">';
      }
    },
